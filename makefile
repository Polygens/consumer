...
export OUTPUT := $(shell echo $$HOME/go/bin/consumer)
...
docker-build:
	docker build -t consumer -t registry.local:5000/consumer:latest -t docker.pkg.github.com/polygens/consumer/consumer:latest --build-arg VERSION=$$VERSION .
...
helm: setup docker-build
	...
	docker push registry.local:5000/consumer:latest
	helm upgrade -i consumer ./charts --set image.repository=registry.local:5000/consumer --set image.pullPolicy=Always --wait --set ingress.enabled=true
